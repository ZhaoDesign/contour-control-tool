...
```bash
python3 scripts/contour_control.py input.png -o output-control.png --face-off
```
...
| `--face` | 单个脸部弱化区域，格式是 `x,y,宽,高` 百分比，可以重复使用 |
| `--face-off` | 关闭所有脸部弱化 |
...
```text
参考图只作为线条控制图使用，只控制人物姿势、身体轮廓、坐姿、手部位置、镜头角度、场景结构和空间关系。多人画面中，每个人的脸部都只以文字提示或对应人物参考图为准，不要复制线条控制图中的原脸。控制图只锁定姿势和场景，不锁定身份。不要生成畸形头部，不要扭曲五官，不要把脸做成原图人物。若某个人脸部弱化未开启，则该人物脸部线条可能会被模型当作身份参考，需要根据目标决定是否开启。
```

## 使用建议

- 如果头部容易坏，把脸部弱化椭圆调大一点。
- 如果是多人图，给每个人单独添加一个脸部弱化区域。
- 如果某个人本来就是身份参考，可以单独关闭这个人的脸部弱化。
